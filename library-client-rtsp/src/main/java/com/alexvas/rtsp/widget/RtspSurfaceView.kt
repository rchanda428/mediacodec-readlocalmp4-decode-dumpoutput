...
//    var file_path: String = "/storage/emulated/0/"
...
//            Log.d(TAG,"Filepath: "+file_path)
            /*val root = File(file_path)
            if (!root.exists()) {
                root.mkdirs()
            }
            val file = File(root.toString() + "/VideoFrameQueueData.txt")
            if (!file.exists()) {
                file.createNewFile()
            }*/
...
//            val directory: File = cw.getDir("imageDir", Context.MODE_PRIVATE)
            fileName = SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().time)
            MyFileWriter.createFile(fileName, context)
...
//                    fos?.write(data)
                    val s1: String = Arrays.toString(data)
                    MyFileWriter.appendData(s1+"\n")
//                    MyFileWriter.appendData(data)
                } else {
    ...
    private fun onRtspClientConnected() {
        ...
        if (videoMimeType.isNotEmpty()) {
            ...
            videoDecodeThread = VideoDecodeThread(
                null, videoMimeType, surfaceWidth, surfaceHeight, videoFrameQueue, onFrameRenderedListener,context)
